...
"password" field is now accepted, which should contain a passphrase::
    ...
    e = create_engine('sqlite+pysqlcipher://:testing@/foo.database')

For an absolute file path, two leading slashes should be used for the
database name::

    e = create_engine('sqlite+pysqlcipher://:testing@//path/to/foo.database')
...
``cipher_page_size`` and ``cipher_use_hmac`` are supported::
    ...
    e = create_engine('sqlite+pysqlcipher://:testing@/foo.database?cipher=aes-256-cfb&kdf_iter=64000')
