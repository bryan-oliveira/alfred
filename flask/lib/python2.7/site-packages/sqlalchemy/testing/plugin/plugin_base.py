...
def setup_options(make_option):
    ...
    make_option("--database", action="append", type="string", dest="database",
...
def _list_dbs(*args):
    print("Available --database options (use --dburi to override)")
    for macro in sorted(file_config.options('database')):
        print("%20s\t%s" % (macro, file_config.get('database', macro)))
    sys.exit(0)
    ...
    if options.db:
        for db_token in options.db:
            for db in re.split(r'[,\s]+', db_token):
                if db not in file_config.options('database'):
                    raise RuntimeError(
                        "Unknown URI specifier '%s'.  "
                        "Specify --dbs for known uris."
                        % db)
                else:
                    db_urls.append(file_config.get('database', db))

    if not db_urls:
        db_urls.append(file_config.get('database', 'default'))
...
def stop_test_class(cls):
    ...
    #assert not inspect(testing.database).get_table_names()
