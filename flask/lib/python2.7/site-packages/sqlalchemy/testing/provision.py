...
def _sqlite_follower_url_from_main(url, ident):
    ...
    if not url.database or url.database == ':memory:':
        ...
        return sa_url.make_url("sqlite:///%s.database" % ident)
    ...
    def connect(dbapi_connection, connection_record):
        ...
        if not follower_ident:
            dbapi_connection.execute(
                'ATTACH DATABASE "test_schema.database" AS test_schema')
        else:
            dbapi_connection.execute(
                'ATTACH DATABASE "%s_test_schema.database" AS test_schema'
                % follower_ident)
...
def _sqlite_drop_db(cfg, eng, ident):
    if ident:
        os.remove("%s_test_schema.database" % ident)
    else:
        os.remove("%s.database" % ident)
...
def _ora_drop_ignore(conn, dbname):
    try:
        ...
        log.info("Reaped database: %s", dbname)
        return True
    except exc.DatabaseError as err:
        log.warning("couldn't drop database: %s", err)
        return False
